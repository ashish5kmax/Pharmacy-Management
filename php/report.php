...
function showSales($start_date, $end_date) {
  ...
  if($con) {
    ...
    $total = 0;
    if($start_date == "" || $end_date == "")
      $query = "SELECT invoices.*, customers.NAME, invoices.INVOICE_ID FROM invoices INNER JOIN customers ON invoices.CUSTOMER_ID = customers.ID";
    else
      $query = "SELECT invoices.*, customers.NAME, invoices.INVOICE_ID FROM invoices INNER JOIN customers ON invoices.CUSTOMER_ID = customers.ID WHERE INVOICE_DATE BETWEEN '$start_date' AND '$end_date'";
